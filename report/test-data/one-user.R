...
path <- paste0(home_path,
         "Haaukernetes/report/test-data/data/one-user/",
         test,
         "/")
...
cpu_plot <- ggplot(df, aes(x = seconds)) +
  ...
  scale_y_continuous(breaks = seq(0, 0.055, 0.0125), limits = c(0, 0.055)) +
  theme_bw() +
...
mem_plot <- ggplot(df, aes(x = seconds)) +
  ...
  scale_x_continuous(breaks = seq(0, s, 20), limits = c(0, s)) +
  scale_y_continuous(breaks = seq(0, 275, 50), limits = c(0, 275)) +
  theme_bw() +
  theme(legend.position = "bottom", text = element_text(size = 14))
